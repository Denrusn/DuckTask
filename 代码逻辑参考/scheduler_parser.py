...
def parse_cn_number(cn_sentence):
    ...
    return ''.join([str(w) for w in words_with_digit2[1:]])
...
class LocalParser(object):
    ...
    def parse_by_rules(self, text):
        _text = re.sub(r'([周|星期]\w)(\d)', r'\1 \2', text, flags=re.U)
    ...
    def consume_repeat(self):
        ...
        if self.consume_word(u'每', u'每隔'):
            ...
            if self.consume_word(u'年') and self.consume_month():
                ...
                if repeat_count < 2:
                    raise ParseError(
                        u'/:no亲，每%s小时的提醒太频繁了，现在还只支持间隔2个小时以上的重复提醒哦~' % repeat_count)
                else:
    ...
    def consume_weekday_period(self):
        ...
        if self.consume_word(u'周', u'下周', u'下个周', u'星期', u'下星期', u'下个星期', u'礼拜', u'下礼拜',
                             u'下个礼拜'):
...
if __name__ == '__main__':
    print(LocalParser().parse_by_rules(u'下午3点45分提醒我吃午饭'))
